...
import { dist, distFast } from '../common/utils';
...
function applyToCircle(circle, cf) {
  ...
  const forceValue = (-circle.radius * cf.radius) / Math.pow(dist, 2) / 100;
...
function draw() {
  ...
  const posAt = (i, j) => {
    const [ax, ay] = field[Math.min(i, gravCols)][Math.min(j, gravRows)];
